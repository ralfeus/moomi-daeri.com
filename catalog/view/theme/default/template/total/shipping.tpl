		...
		success: function(json) {
			...
			if (json['error']) {
				if (json['error']['warning']) {
					$('#basket').before('<div class="error">' + json['error']['warning'] + '</div>');
				}
			...
			if (json['shipping_methods']) {
              ...
              var desc = jQuery.parseJSON(json['shipping_methods'][i]['quote'][j]['description']);
              var lang = '<?php echo $this->language->get('code'); ?>';
              if(desc == null) {
                desc = {};
              }
              if(typeof(desc[lang]) == 'undefined') {
                desc[lang] = '';
              }
							...
							html += '  <td><label for="' + json['shipping_methods'][i]['quote'][j]['code'] + '">' + json['shipping_methods'][i]['quote'][j]['title'];
              if(desc[lang] != '') {
                html += " - " + desc[lang];
              }
              html += '</label></td>';
...
$('#button-shipping').live('click', function() {
	$.ajax({
		...
		success: function(json) {
			if (json['error']) {
				$('#shipping').before('<div class="error">' + json['error'] + '<img src="catalog/view/theme/default/image/close.png" alt="" class="close" /></div>');
			}
