...
class ModelAccountReturn extends Model {
	...
	public function getReturn($return_id) {
		$query = $this->db->query("SELECT * FROM `return`WHERE return_id = '" . (int)$return_id . "' AND customer_id = '" . $this->customer->getId() . "'");
	...
	public function getTotalReturnProductsByReturnId($return_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM return_product WHERE return_id = '" . (int)$return_id . "'");
	...
	public function getReturnHistories($return_id) {
		$query = $this->db->query("SELECT rh.date_added, rs.name AS status, rh.comment, rh.notify FROM return_history rh LEFT JOIN return_status rs ON rh.return_status_id = rs.return_status_id WHERE rh.return_id = '" . (int)$return_id . "' AND rs.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY rh.date_added ASC");
