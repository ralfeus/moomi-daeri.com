...
class ModelCheckoutOrder extends Model {	
		...
		foreach ($order_items as $order_item)
		{
			$this->db->query("
				...
				VALUES(" . 
					...
					$this->db->escape($order_item['name']) . "', '" . 
					$this->db->escape($order_item['model']) . "', " .
					(int)$order_item['quantity'] . ", " .
					(float)$order_item['price'] . ", " .
					(float)$order_item['total'] . ", " .
					(float)$order_item['tax'] . ")
			");			
			$order_item_id = $this->db->getLastId();
			...
			if (count($order_item['options']))
			{
				...
				foreach ($order_item['options'] as $option) 
				{
					$records .= "(" .
						...
						$this->db->escape($option['name']) . "', '" .
						$this->db->escape($option['option_value']) . "', '" .
						$this->db->escape($option['type']) . "'
					),";
				}
			...
			if (count($order_item['download'])) 
			{
				...
				foreach ($order_item['download'] as $download) 
				{
					$records .= "VALUES(" .
						(int)$orderId . ", " .
						(int)$order_item_id . ", '" .
						$this->db->escape($download['name']) . "', '" .
						$this->db->escape($download['filename']) . "', '" .
						$this->db->escape($download['mask']) . "', " .
	...
	public function create($data) {
		$this->db->query("
			...
			SET 
				...
				payment_firstname = '" . $this->db->escape($data['shipping_firstname']) . "', 
				payment_lastname = '" . $this->db->escape($data['shipping_lastname']) . "', 
				payment_company = '" . $this->db->escape($data['shipping_company']) . "', 
				payment_address_1 = '" . $this->db->escape($data['shipping_address_1']) . "', 
				payment_address_2 = '" . $this->db->escape($data['shipping_address_2']) . "', 
				payment_city = '" . $this->db->escape($data['shipping_city']) . "', 
				payment_postcode = '" . $this->db->escape($data['shipping_postcode']) . "', 
				payment_country = '" . $this->db->escape($data['shipping_country']) . "', 
				payment_country_id = '" . (int)$data['shipping_country_id'] . "', 
				payment_zone = '" . $this->db->escape($data['shipping_zone']) . "', 
				payment_zone_id = '" . (int)$data['shipping_zone_id'] . "', 
				payment_address_format = '" . $this->db->escape($data['shipping_address_format']) . "', 
				payment_method = '" . $this->db->escape($data['shipping_method']) . "', 
    ...
    private function sendMail($orderInfo, $comment, $notify)
    {
        ...
        $order_status_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_status WHERE order_status_id = '" . (int)$orderInfo['order_status_id'] . "' AND language_id = '" . (int)$orderInfo['language_id'] . "'");
        ...
        $order_product_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = " . (int)$orderInfo['order_id']);
        $order_total_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_total WHERE order_id = " . (int)$orderInfo['order_id'] . " ORDER BY sort_order ASC");
        $order_download_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_download WHERE order_id = " . (int)$orderInfo['order_id']);
        ...
        foreach ($order_product_query->rows as $product) {
            ...
            $order_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$orderInfo['order_id'] . "' AND order_product_id = '" . (int)$product['order_product_id'] . "'");
        ...
        foreach ($order_product_query->rows as $result) {
            ...
            $order_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$orderInfo['order_id'] . "' AND order_product_id = '" . $result['order_product_id'] . "'");
