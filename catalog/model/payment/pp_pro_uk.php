...
class ModelPaymentPPProUK extends \system\engine\Model {
  	public function getMethod($address, $total) { 
		...
		$query = $this->getDb()->query("SELECT * FROM zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('pp_pro_uk_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
