...
class ControllerAccountRegister extends Controller {
    ...
    public function __construct($registry)
    {
      ...
      $this->data['publicKey'] = $this->config->get('RECAPTCHA_PUBLIC_KEY');//RECAPTCHA_PUBLIC_KEY;

    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate())
        {
			unset($this->session->data['guest']);
            try {$this->modelAccountCustomer->addCustomer($this->request->post);}
            catch (Exception $exc)
            {
                $this->session->data['notifications']['warning'] = "Couldn't send a mail";
            }
        ...
        $this->data['textBillingDetails'] = $this->language->get('BILLING_DETAILS');
      ...
      $this->data['error_recaptcha'] = '';
        ...
        foreach ($this->modelLocalisationCurrency->getCurrencies() as $currency)
        {
          $this->data['currencies'][] = array(
              ...
              'name' => $currency['title'],
              'selected' => ($currency['code'] == $this->parameters['baseCurrency']) ? "selected=\"true\"" : ""
          );
        }

        $templateName = '/template/account/register.tpl';
    ...
    {
        $this->parameters['baseCurrency'] = empty($_REQUEST['baseCurrency']) ? 'EUR' : $_REQUEST['baseCurrency'];
    }
      ...
      if (!$resp->is_valid) {
        $this->error['recaptcha'] = $this->language->get('Ебать тут код не правильный');
      }
      ...
      if (!isset($this->request->post['firstname']) || (utf8_strlen($this->request->post['firstname']) > 32)) {
    	...
    	if (!isset($this->request->post['lastname']) || (utf8_strlen($this->request->post['lastname']) > 32)) {
      		$this->error['lastname'] = $this->language->get('error_lastname');
    	}

        if (!isset($this->request->post['nickname']) || $this->model_account_customer->getCustomerByNickname(($this->request->post['nickname'])))
