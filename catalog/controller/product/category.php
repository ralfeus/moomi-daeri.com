...
class ControllerProductCategory extends Controller {
    protected function initParameters() {
        $this->initParametersWithDefaults([
            'order' => ['DESC', function ($value) {
                return isset($value) && in_array($value, ['ASC', 'DESC']);
            }],
            'sort' => ['p.date_added', function ($value) {
                return isset($value) && in_array($value, ['p.date_added']);
            }],
            'page' => [1, function ($value) {
                return isset($value) && is_numeric($value);
            }],
            'limit' => [$this->getConfig()->get('config_catalog_limit'), function ($value) {
                return isset($value) && is_numeric($value);
            }],
            'path' => null
        ]);
...
//		}
        ...
        if ($category) {
            ...
            $product_total = ProductDAO::getInstance()->getProductsCount($filter, true);
            $results = ProductDAO::getInstance()->getProducts($filter,
                $this->parameters['sort'], $this->parameters['order'],
                ($this->parameters['page'] - 1) * $this->parameters['limit'], $this->parameters['limit'], true, true);
            foreach ($results as $result) {
                ...
                if (($this->getConfig()->get('config_customer_price') && $this->customer->isLogged()) || !$this->getConfig()->get('config_customer_price')) {
                    $price = $this->getCurrentCurrency()->format($result->getPrice());
                } else {
                    $price = false;
                }

                if ((float)$result->getSpecialPrice($this->getCurrentCustomer()->getCustomerGroupId())) {
                    $special = $this->getCurrentCurrency()->format($result->getSpecialPrice($this->getCurrentCustomer()->getCustomerGroupId()));
                } else {
                    $special = false;
                }
...
//				}
            ...
            $this->data['limit'] = $this->parameters['limit'];
            ...
            $templateDir = file_exists(DIR_TEMPLATE . $this->getConfig()->get('config_template') . $templateName)
                ? $this->getConfig()->get('config_template')
                : 'default';
            $this->getResponse()->setOutput($this->render($templateDir . $templateName));
        } else {
            $url = '';
            ...
            $templateName = '/template/error/not_found.tpl';
            $templateDir = (file_exists(DIR_TEMPLATE . $this->getConfig()->get('config_template') . $templateName))
                ? $this->getConfig()->get('config_template')
                : 'default';
            $this->getResponse()->setOutput($this->render($templateDir . $templateName));
