        ...
        foreach ($this->session->data['cart'] as $key => $cartItem) {
            ...
            $product_query = $this->getDb()->query("
      		    ...
      		    FROM
      		        ...
      		        LEFT JOIN supplier_to_manufacturer AS s2m ON p.supplier_id = s2m.supplier_id
      		        LEFT JOIN manufacturer AS m ON s2m.manufacturer_id = m.manufacturer_id
      		        LEFT JOIN product_description pd ON (p.product_id = pd.product_id)
                WHERE
                    p.product_id = ?
                    AND pd.language_id = ?
                    AND p.date_available <= ? AND p.status = '1'
                ", array(
                    "i:$product_id",
            ...
            if ($product_query->num_rows) {
                ...
                $product_data[$key] = array(
                    ...
                    'brand' => $product_query->row['brand'],
                    'supplierId' => $product_query->row['supplier_id'],
                    'supplierShippingCost' => $product_query->row['shipping_cost'],
                    'supplierFreeShippingThreshold' => $product_query->row['free_shipping_threshold']
                );
            } else {
                $this->remove($key);
            }
