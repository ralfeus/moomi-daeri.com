...
class GeneralDAO extends DAO {
    ...
    public function addHoliday($data) {
        ...
        $this->getDb()->query("INSERT INTO shop_holiday SET start = '" . $this->getDb()->escape($data['start']) . "', end = '" . $this->getDb()->escape($data['end']) . "', name = '" . $this->getDb()->escape(strip_tags($data['name'])) . "'");
    ...
    public function deleteHoliday($holiday_id) {
        ...
        $this->getDb()->query("DELETE FROM shop_holiday WHERE holiday_id = '" . $holiday_id . "'");

    }

    public function getAllHolidays() {

        $query = $this->getDb()->query("SELECT * FROM shop_holiday");
