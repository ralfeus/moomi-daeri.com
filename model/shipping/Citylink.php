...
class Citylink extends ShippingMethodBase {
...
//        $this->log->write(print_r($address, true));
        $sql = "
            ...
            FROM
                setting AS s
                JOIN geo_zone AS gz ON `key` = concat('citylink_', gz.geo_zone_id, '_status') AND value = 1
        ...
        if ($query->row)
        {
            ...
            {
                $row['code'] = 'citylink.citylink_' . $row['geo_zone_id'];
                $row['shippingMethodName'] = 'citylink Shipping ' . $row['name'];
                $result[] = $row;
            }
            return $result;
        }
    ...
    public function getQuote($address) {
		$this->getLoader()->language('shipping/citylink');
		...
		foreach ($query->rows as $result) {
			if ($this->getConfig()->get('citylink_' . $result['geo_zone_id'] . '_status')) {
			...
			if ($status) {
				...
				$rates = explode(',', $this->getConfig()->get('citylink_' . $result['geo_zone_id'] . '_rate'));
				...
				if ((string)$cost != '') {
					$quote_data['citylink_' . $result['geo_zone_id']] = array(
						'code'         => 'citylink.citylink_' . $result['geo_zone_id'],
						...
						'cost'         => $cost,
						'tax_class_id' => $this->getConfig()->get('citylink_tax_class_id'),
						'text'         => $this->currency->format($this->tax->calculate($cost, $this->getConfig()->get('citylink_tax_class_id'), $this->getConfig()->get('config_tax')))
					);
				}
			}
		}
		...
		if ($quote_data) {
      		$method_data = array(
        		...
        		'quote'      => $quote_data,
				'sort_order' => $this->getConfig()->get('citylink_sort_order'),
        		'error'      => false
      		);
		}
	...
	public function isEnabled() {
		return $this->getConfig()->get('citylink_status');
	}

	public function getSortOrder() {
		return $this->getConfig()->get('citylink_sort_order');
	}
}
