...
class Ems extends ShippingMethodBase {
...
//        $this->log->write(print_r($address, true));
    ...
    {
        return $this->getNameByResource('shipping/ems');
    }

    public function getQuote($address) {
        $this->getLoader()->language('shipping/ems');
        ...
        foreach ($query->rows as $result) {
            if ($this->getConfig()->get('ems_' . $result['geo_zone_id'] . '_status')) {
            ...
            if ($status) {
                ...
                $rates = explode(',', $this->getConfig()->get('ems_' . $result['geo_zone_id'] . '_rate'));
                ...
                if ((string)$cost != '') {
                    $quote_data['ems_' . $result['geo_zone_id']] = array(
                        ...
                        'cost'         => $cost,
                        'tax_class_id' => $this->getConfig()->get('ems_tax_class_id'),
                        'text'         => $this->currency->format($this->tax->calculate($cost, $this->getConfig()->get('ems_tax_class_id'), $this->getConfig()->get('config_tax')))
                    );
                }
            }
        }
        ...
        if ($quote_data) {
            $method_data = array(
                ...
                'quote'      => $quote_data,
                'sort_order' => $this->getConfig()->get('ems_sort_order'),
                'error'      => false
            );
        }
    ...
    public function isEnabled() {
        return $this->getConfig()->get('ems_status');
    }

    public function getSortOrder() {
        return $this->getConfig()->get('ems_sort_order');
    }
}