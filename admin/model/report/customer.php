...
class ModelReportCustomer extends \system\engine\Model {
	...
	public function getRewardPoints($data = array()) { 
		$sql = "SELECT cr.customer_id, CONCAT(c.firstname, ' ', c.lastname) AS customer, c.email, cg.name AS customer_group, c.status, SUM(cr.points) AS points, COUNT(o.order_id) AS orders, SUM(o.total) AS total FROM customer_reward cr LEFT JOIN `customer` c ON (cr.customer_id = c.customer_id) LEFT JOIN customer_group cg ON (c.customer_group_id = cg.customer_group_id) LEFT JOIN `order` o ON (cr.order_id = o.order_id)";
	...
	public function getCredit($data = array()) { 
		$sql = "SELECT ct.customer_id, CONCAT(c.firstname, ' ', c.lastname) AS customer, c.email, cg.name AS customer_group, c.status, SUM(ct.amount) AS total FROM customer_transaction ct LEFT JOIN `customer` c ON (ct.customer_id = c.customer_id) LEFT JOIN customer_group cg ON (c.customer_group_id = cg.customer_group_id)";
