<?php 
class ModelLocalisationTaxRate extends \system\engine\Model {
	public function addTaxRate($data) {
		$this->db->query("INSERT INTO tax_rate SET name = '" . $this->db->escape($data['name']) . "', rate = '" . (float)$data['rate'] . "', `type` = '" . $this->db->escape($data['type']) . "', geo_zone_id = '" . (int)$data['geo_zone_id'] . "', date_added = NOW()");
	...
	public function getTaxRateCustomerGroups($tax_rate_id) {
		...
		$query = $this->db->query("SELECT * FROM tax_rate_to_customer_group");
