...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		if (isset($data['product_option'])) {
            $color_option_id = $this->config->get('config_color_option_id');
			foreach ($data['product_option'] as $product_option) {
				if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
                    $this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', required = '" . (int)$product_option['required'] . "', parent_option_id = '" . (int)$product_option['parent_option_id'] . "'");

					$product_option_id = $this->db->getLastId();
				
					if (isset($product_option['product_option_value'])) {
                        if($product_option['option_id'] == $color_option_id) {
                            foreach ($product_option['product_option_value'] as $key => $product_option_value) {
                                $product_option['product_option_value'][$key]['option_value_id'] = $this->checkOptionByName($product_option_value['option_value_id'], $color_option_id);
                            }
                        }
                        foreach ($product_option['product_option_value'] as $product_option_value) {
                            $parent_option_value = implode(',', $product_option_value['parent_option_value']);
                            $this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', option_value_id = '" . (int)$product_option_value['option_value_id'] . "', parent_option_value = '" . $this->db->escape($parent_option_value) . "', quantity = '" . (int)$product_option_value['quantity'] . "', subtract = '" . (int)$product_option_value['subtract'] . "', price = '" . (float)$product_option_value['price'] . "', price_prefix = '" . $this->db->escape($product_option_value['price_prefix']) . "', points = '" . (int)$product_option_value['points'] . "', points_prefix = '" . $this->db->escape($product_option_value['points_prefix']) . "', weight = '" . (float)$product_option_value['weight'] . "', weight_prefix = '" . $this->db->escape($product_option_value['weight_prefix']) . "'");
                        }
                    }
				} else { 
    ...
    public function checkOptionByName($name, $option_id) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "option_value_description WHERE option_id = '" . $option_id . "' AND LCASE(name) LIKE '%" . $this->db->escape(strtolower($name)) . "%'");
        if($query->rows) {
            ...
            foreach($languages as $language) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "option_value_description SET option_value_id = '" . (int)$option_value_id . "', name = '" . $name . "', option_id = '" . $option_id . "', language_id = '" . $language['language_id'] . "'");
            }
    ...
    public function editProduct($product_id, $data) {
		...
		if (isset($data['product_option'])) {
            ...
            foreach ($data['product_option'] as $product_option) {
				if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
                    $this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET product_option_id = '" . (int)$product_option['product_option_id'] . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', required = '" . (int)$product_option['required'] . "', parent_option_id = '" . (int)$product_option['parent_option_id'] . "'");
					$product_option_id = $this->db->getLastId();
					...
					if (isset($product_option['product_option_value'])) {
                        ...
                        foreach ($product_option['product_option_value'] as $product_option_value) {
                            ...
                            $parent_option_value = implode(',', $product_option_value['parent_option_value']);
                            $this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET product_option_value_id = '" . (int)$product_option_value['product_option_value_id'] . "', product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', option_value_id = '" . (int)$product_option_value['option_value_id'] . "', parent_option_value = '" . $this->db->escape($parent_option_value) . "', quantity = '" . (int)$product_option_value['quantity'] . "', subtract = '" . (int)$product_option_value['subtract'] . "', price = '" . (float)$product_option_value['price'] . "', price_prefix = '" . $this->db->escape($product_option_value['price_prefix']) . "', points = '" . (int)$product_option_value['points'] . "', points_prefix = '" . $this->db->escape($product_option_value['points_prefix']) . "', weight = '" . (float)$product_option_value['weight'] . "', weight_prefix = '" . $this->db->escape($product_option_value['weight_prefix']) . "'");
                        }
					}
				} else { 
		...
		foreach ($product_option_query->rows as $product_option) {
			if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
				...
				foreach ($product_option_value_query->rows as $product_option_value) {
                    $parent_option_value = array();
                    $parent_option_value = explode(',', $product_option_value['parent_option_value']);
                    $product_option_value_data[] = array(
