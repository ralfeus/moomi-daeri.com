...
class ModelCatalogDownload extends Model {
	public function addDownload($data) {
      	...
      	if (isset($data['download'])) {
        	$this->db->query("UPDATE download SET filename = '" . $this->db->escape($data['download']) . "', mask = '" . $this->db->escape($data['mask']) . "' WHERE download_id = '" . (int)$download_id . "'");
      	}
	...
	public function editDownload($download_id, $data) {
        $query = $this->db->query("SELECT filename from download WHERE download_id = '" . (int)$download_id . "'");
		...
		if (isset($data['download'])) {
        	...
        	if (isset($data['update'])) {
      			...
      			$this->db->query("UPDATE order_download SET remaining = '" . (int)$query->row['remaining'] . "', `filename` = '" . $this->db->escape($query->row['filename']) . "', mask = '" . $this->db->escape($query->row['mask']) . "' WHERE `filename` = '" . $this->db->escape($old_filename) . "'");
      		}
