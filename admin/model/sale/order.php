		...
		if ($currency_info) {
      	...
      	if (isset($data['order_total'])) {
      		foreach ($data['order_total'] as $order_total) {
      			$this->db->query("INSERT INTO order_total SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$return_product['product_id'] . "', name = '" . $this->db->escape($return_product['name']) . "', model = '" . $this->db->escape($return_product['model']) . "', quantity = '" . (int)$return_product['quantity'] . "', manufacturer = '" . (int)$return_product['manufacturer'] . "', return_reason_id = '" . (int)$return_product['return_reason_id'] . "', opened = '" . (int)$return_product['opened'] . "', comment = '" . $this->db->escape($return_product['comment']) . "', return_action_id = '" . (int)$return_product['return_action_id'] . "'");
			}
		}
	}
    ...
    private function buildFilterString($data = array()) {
        ...
        else
        {
            if (!empty($data['filter_date_added']))
                $filter .= ($filter ? " AND " : "") . "DATE(o.date_added) = DATE('" . $this->db->escape($data['filter_date_added']) . "')";
            if (!empty($data['filter_customer']))
                $filter .= ($filter ? " AND " : "") . "LCASE(CONCAT(o.firstname, ' ', o.lastname)) LIKE '" . $this->db->escape(utf8_strtolower($data['filter_customer'])) . "%'";
    ...
    public function getShippingAddressId($orderId)
    {
        ...
        if ($query->num_rows) // there is such order
        {
            ...
            else // if order doesn't have address ID it's necessary to create one from existing address in order
            {
                ...
                $orderAddressId = 0;
                foreach ($customerModel->getAddresses($order['customer_id']) as $customerAddress) // go through customer's addresses
                {
