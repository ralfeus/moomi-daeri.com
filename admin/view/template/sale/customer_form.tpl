...
function deleteTransaction(transactionId) {
    if (!confirm('Transaction deletion cannot be undone! Are you sure you want to do this?'))
        return false;
    $.ajax({
        type: 'POST',
        url: 'index.php?route=sale/customer/transaction&token=<?php echo $token; ?>&customer_id=<?php echo $customer_id; ?>',
        ...
        beforeSend: function() {
            $('.success, .warning').remove();
            $('#button-transaction').attr('disabled', true);
            $('#transaction').before('<div class="warning"><img src="view/image/loading.gif" alt="" /> <?= $text_wait ?></div>');
        },
        complete: function() {
            $('#button-transaction').attr('disabled', false);
            $('.warning').remove();
...
function addRewardPoints() {
	$.ajax({
		...
		beforeSend: function() {
			...
			$('#reward').before('<div class="error"><img src="view/image/loading.gif" alt="" /> <?php echo $text_wait; ?></div>');
		},
		complete: function() {
