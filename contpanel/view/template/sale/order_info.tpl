...
$('#invoice-create').live('click', function() {
	$.ajax({
		...
		success: function(json) {
			...
			if (json['error']) {
				$('#tab-order').prepend('<div class="error" style="display: none;">' + json['error'] + '</div>');
			...
			if (json.invoice_no) {
				...
				$('#order-status').fadeOut('slow', function() {
					$('#order-status').html(json['order_status']);
					$('#order-status').fadeIn('slow');
				});
			}
		}
	});
...
$('#credit-add').live('click', function() {
	$.ajax({
		...
		success: function(json) {
			...
			if (json['error']) {
				$('.breadcrumb').after('<div class="error" style="display: none;">' + json['error'] + '</div>');
...
function history() {
	$.ajax({
		...
		beforeSend: function() {
			...
			$('#history').prepend('<div class="error"><img src="view/image/loading.gif" alt="" /> <?php echo $text_wait; ?></div>');
		},
		complete: function() {
