...
class ModelSaleReturn extends Model {
	...
	public function getReturns($data = array()) {
		...
		if (!empty($data['filter_quantity'])) {
			$implode[] = "(SELECT SUM(rp.quantity) FROM return_product rp WHERE rp.return_id = r.return_id GROUP BY rp.return_id) = '" . (int)$data['filter_quantity'] . "'";
		}	
	...
	public function editReturnProduct($return_id, $return_product_id, $return_action_id) {
		$this->db->query("UPDATE return_product SET return_action_id = '" . (int)$return_action_id . "' WHERE return_id = '" . (int)$return_id . "' AND return_product_id = '" . (int)$return_product_id . "'");
	}
	
	public function getReturnProducts($return_id) {
		$query = $this->db->query("SELECT *, (SELECT rr.name FROM return_reason rr WHERE rr.return_reason_id = rp.return_reason_id AND rr.language_id = '" . (int)$this->config->get('config_language_id') . "') AS reason, (SELECT ra.name FROM return_action ra WHERE ra.return_action_id = rp.return_action_id AND ra.language_id = '" . (int)$this->config->get('config_language_id') . "') AS action FROM return_product rp WHERE rp.return_id = '" . $return_id . "'");
		
		return $query->rows;	
	}	
	
	public function getTotalReturnProducts($return_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `return`WHERE return_id = '" . (int)$return_id . "'");
		
		return $query->rows;	
	}
			
	public function getTotalReturnProductsByReturnReasonId($return_reason_id) {
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM return_product WHERE return_reason_id = '" . (int)$return_reason_id . "'");
				
		return $query->row['total'];
	}	
	
	public function getTotalReturnProductsByReturnActionId($return_action_id) {
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM return_product WHERE return_action_id = '" . (int)$return_action_id . "'");
	...
	public function getTotalReturnHistoriesByReturnStatusId($return_status_id) {
	  	$query = $this->db->query("SELECT COUNT(*) AS total FROM return_history WHERE return_status_id = '" . (int)$return_status_id . "' GROUP BY order_id");
