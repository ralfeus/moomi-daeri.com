...
class ModelUserUser extends Model {
	...
	public function editPassword($user_id, $password) {
		$this->db->query("UPDATE `user` SET password = '" . $this->db->escape(md5($password)) . "' WHERE user_id = '" . (int)$user_id . "'");
	}

	public function editCode($email, $code) {
		$this->db->query("UPDATE `user` SET code = '" . $this->db->escape($code) . "' WHERE email = '" . $this->db->escape($email) . "'");
	}
	...
	public function getTotalUsersByEmail($email) {
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM `user` WHERE email = '" . $this->db->escape($email) . "'");
