...
class ControllerTotalLocalShipping extends Controller {
	...
	public function index() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			$this->model_setting_setting->editSetting('localShipping', $this->request->post);
		...
		$this->data['button_cancel'] = $this->language->get('button_cancel');
 		...
 		if (isset($this->error['warning'])) {
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('total/localShipping', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('total/localShipping', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->template = 'total/localShipping.tpl.php';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'total/localShipping')) {
